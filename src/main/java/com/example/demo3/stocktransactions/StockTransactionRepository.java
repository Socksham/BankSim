...
public interface StockTransactionRepository extends JpaRepository<StockTransaction, Long> {
    @Query("SELECT c FROM StockTransaction c WHERE concat(c.bank.id, '') LIKE concat('%', :bank, '%') AND c.stock = :name")
    List<StockTransaction> findByName(@Param("bank") Bank bank, @Param("name") String name);

    @Query("SELECT c FROM StockTransaction c WHERE concat(c.bank.id, '') LIKE concat('%', :bank, '%') AND c.stock = :transaction")
